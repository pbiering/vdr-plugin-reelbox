...
namespace Reel
{
    ...
    void AudioOut::Action()
    {
        ...
        SystemExec("amixer cset `amixer controls |grep 'IEC958'|grep 'Playback Switch'|cut -d , -f 1` 1 > /dev/null\n");
        SystemExec("amixer cset `amixer controls |grep 'IEC958'|grep 'Playback Switch'|cut -d , -f 1` 0 > /dev/null\n");
