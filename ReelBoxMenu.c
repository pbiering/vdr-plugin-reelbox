...
:liteHasHdext_(false)
{
    ...
    /*if (RBSetup.HDoutput == HD_OUTPUT_BOTH)
	expert_mode = true;
    else */
    expert_mode = false;
...
void cMenuVideoMode::Setup()
{
...
#else
            Add(new cMenuEditStraItem(tr("Output (HDMI_ONLY active)"), &rbSetup_.HDoutput, numberOutputs, output));
#endif
	    /*
	...
	//FIX THIS	rbSetup_.HDamode = HD_APORT_SCART;
...
#else
                Add(new cMenuEditStraItem(tr("Resolution (HD_ONLY active)"), &rbSetup_.HDresolution, 2, showResolution));
#endif
		...
		Add(new cMenuEditStraItem(tr("Refresh Rate"), &rbSetup_.HDnorm, 3, showNorm));
            ...
            if (rbSetup_.HDresolution == HD_VM_RESOLUTION_576) rbSetup_.HDnorm = 0;
		...
		if (rbSetup_.HDresolution != HD_VM_RESOLUTION_720)
            ...
            trclOsd[0]   = tr("256 Colors OSD");
            trclOsd[1]   = tr("True Color OSD");

            Add (new cMenuEditStraItem(tr("OSD type"), &rbSetup_.TRCLosd, 2, trclOsd));
...
void cMenuVideoMode::Store(void)
{
    ...
    plugin->SetupStore("TRCLosd",      RBSetup.TRCLosd);
    ...
    plugin->SetupStore("HDoverscan",     RBSetup.HDoverscan);
